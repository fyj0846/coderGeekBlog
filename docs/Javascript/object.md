...
## 属性访问和变量访问
### 属性访问
对象的默认[[get]]操作首先在对象中查找是否有名称相同的属性，如果找到就返回，如果没找到，就沿着原型链[[prototype]]往上搜索，直到找到或者到达原型链的顶端
...
## 其他
### Javascript中的类
Javascript中只有对象。在继承或实例化时，Javascript的对象并不会执行复制，也不存在可以被实例化的“类”。一个对象不会被复制到其他对象，只会被关联。为了模拟复制，引入了“混入”。